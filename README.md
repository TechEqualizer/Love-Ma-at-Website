...
|---|---|---|
| `maat-portrait.webp` | `portrait-original.jpeg` | background removed (`rembg`), trimmed, resized |
| `logo-mark.png` / `logo-word.png` / `logo-full.png` | `logo-original.jpg` | white background keyed out and un-matted to transparency |
| `portrait-cutout.png` (in `brand/`) | — | full-resolution cutout master, kept but not shipped |
